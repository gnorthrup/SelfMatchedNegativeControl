...
for (person in 1:nrow(india.people)){
  ...
  if(sum(subset(india.total,id==india.people[person,"id"],select=rv))<4){
    next
  } else {
    india.four.rota <- c(india.four.rota,india.people[person,"id"])
  }
}
india.times.list <- c()
person.num <- 1
for (person in india.four.rota){
...
for (person in 1:nrow(mexico.people)){
  #print(india.people[person,"id"])
  if(sum(subset(mexico.total,id==mexico.people[person,"id"],select=rv))<4){
    next
  } else {
    mexico.four.rota <- c(mexico.four.rota,mexico.people[person,"id"])
  }
}
mexico.times.list <- c()
person.num <- 1
for (person in mexico.four.rota){
...
protect2prior <- c('lower'=lower,'est'=real.est,'upper'=upper)

save(protect2prior,file='protect2prior.Rdata')
